...
Decryptor::Decryptor(JNIEnv* env, const unsigned char passwordData[], int passwordSize) {
	this->env = env;

	SetKey(passwordData, passwordSize);
	Init();
}
...
void Decryptor::SetKey(const unsigned char passwordData[], int passwordSize) {
	jclass messageDigestClass = env->FindClass("java/security/MessageDigest");
	jmethodID getInstance = env->GetStaticMethodID(messageDigestClass, "getInstance", "(Ljava/lang/String;)Ljava/security/MessageDigest;");
	jmethodID digest = env->GetMethodID(messageDigestClass, "digest", "([B)[B");
	...
	jbyteArray keyData = CharArrayToJavaByteArray(env, passwordData, passwordSize);
